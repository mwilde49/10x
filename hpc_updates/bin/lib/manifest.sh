...
# For native pipelines, pass "native:<tool_path>" to record tool version instead.
compute_sif_checksum() {
    ...
    if [[ "$sif" == native:* ]]; then
        ...
        local binary="$tool_dir/$tool_name"
        if [[ -x "$binary" ]]; then
            "$binary" --version 2>/dev/null | head -1 || printf 'version_unknown'
        else
            printf 'binary_not_found'
        fi
    elif [[ -f "$sif" ]]; then
...
# For native pipelines, container_path should be "native:<tool_path>".
generate_manifest() {
    local run_dir="$1"
    local pipeline="$2"
    local config="$3"
    ...
    # Stable JSON schema — same field names for all pipelines.
    # For native pipelines, container_file holds "native:<tool_path>"
    # and container_checksum holds the tool version string.
